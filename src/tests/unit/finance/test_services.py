...
class TestOwnerSettlement:
    ...
    def test_splits_above_floor_cash(self):
        ...
        payment = PaymentFactory(
            lease__owner_agreement=agreement, amount=Decimal("700.00"), currency=Currency.USD,
            payment_date=date(2026, 6, 10), due_date=date(2026, 6, 1), rental_period=date(2026, 6, 1),
    ...
    def test_partial_cash_keeps_floor_and_records_only_uncovered_exposure(self):
        ...
        payment = PaymentFactory(
            lease__owner_agreement=agreement, amount=Decimal("300.00"), currency=Currency.USD,
            payment_date=date(2026, 6, 10), due_date=date(2026, 6, 1), rental_period=date(2026, 6, 1),
            status=PaymentStatus.PAID,
        )
    ...
    def test_deposit_never_affects_settlement(self):
        ...
        deposit = PaymentFactory(
            lease__owner_agreement=agreement, amount=Decimal("900.00"), kind=PaymentKind.DEPOSIT,
            payment_date=date(2026, 6, 10), due_date=date(2026, 6, 1), rental_period=date(2026, 6, 1),
            status=PaymentStatus.PAID,
        )
    ...
    def test_late_cash_after_paid_base_creates_upside_adjustment(self):
        ...
        receipt = PaymentFactory(
            lease__owner_agreement=agreement, amount=Decimal("700.00"), currency=Currency.USD,
            payment_date=date(2026, 7, 3), due_date=date(2026, 6, 1), rental_period=date(2026, 6, 1),
            status=PaymentStatus.PAID,
        )
    ...
    def test_prorates_first_month_and_is_idempotent(self):
        agreement = OwnerAgreementFactory(
            start_date=date(2026, 6, 16), end_date=date(2026, 6, 30), gross_floor_amount=Decimal("600.00"),
    ...
    def test_prorating_rounds_to_money_precision(self):
        agreement = OwnerAgreementFactory(
            start_date=date(2026, 7, 17), end_date=date(2026, 7, 31), gross_floor_amount=Decimal("600.00"),
            commission_rate=Decimal("10.00"),
        )
